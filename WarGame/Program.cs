    ...
    class Program
    {
        ...
        SA.Value = 14;
            ...
            while ((PlayerCards.Count > 0) && (ComputerCards.Count > 0))
            {
                ...
                if (DrawNumber > 0)
                {
                    Console.WriteLine("DrawNumber = "+DrawNumber);
                    DrawCCrand = rand.Next(ComputerCards.Count);
                    DrawPCrand = rand.Next(PlayerCards.Count);
                    Console.WriteLine("Because of Draw, Player is risking "+ PlayerCards[DrawPCrand].Code + " card and Computer is risking " + ComputerCards[DrawCCrand].Code);
                    DrawDeck.Add(ComputerCards[DrawCCrand]);
                    Console.WriteLine("Adding "+ ComputerCards[DrawCCrand].Code + " to DrawDeck");
                    DrawDeck.Add(PlayerCards[DrawPCrand]);
                    Console.WriteLine("Adding " + PlayerCards[DrawPCrand].Code + " to DrawDeck");
                ...
                PCrand = rand.Next(PlayerCards.Count);
                    ...
                    if (DrawNumber > 0)
                    {
                        ...
                        {
                            Console.WriteLine("Adding do PlayerCards: " + DrawDeck[i].Code);
                            PlayerCards.Add(DrawDeck[i]);
                            
                        }
                        for (int i = DrawCount-1; i >= 0; i--)
                        {
                            Console.WriteLine("Removing from DrawDeck: " + DrawDeck[i].Code);
                ...
                else
                {
                    ...
                    if (DrawNumber > 0)
                    {
                        ...
                        for (int i = 0; i < DrawCount; i++)
                        {
                            Console.WriteLine("Adding do ComputerCards: " + DrawDeck[i].Code);
